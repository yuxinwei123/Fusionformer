...
def save_model_2(previous_name, save_dir, epoch, data_threshold, model,model_name):
    if os.path.exists(previous_name):
        os.remove(previous_name)

    torch.save(model.state_dict(),
               '%s/model_%s_%d_%d.pth' % (save_dir, model_name, epoch, data_threshold * 100))
    previous_name = '%s/model_%s_%d_%d.pth' % (save_dir,model_name, epoch, data_threshold * 100)
    return previous_name
    

def save_model_pretrain(previous_name, save_dir, epoch, data_threshold, model,model_name):
    if os.path.exists(previous_name):
        os.remove(previous_name)
