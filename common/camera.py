...
def get_uvd2xyz(uvd, gt_3D, cam):
    ...
    root = gt_3D[:, :, 0, :].unsqueeze(-2).repeat(1, 1, V, 1).clone()#取gt_3D根节点，扩展
    enc_in_all = uvd[:, :, :, :2].view(-1, T, V, 2).clone() #取uvd的x,y

    cam_f_all = cam[..., :2].view(-1,1,1,2).repeat(1,T,V,1)#前两个相机参数，扩展
    cam_c_all = cam[..., 2:4].view(-1,1,1,2).repeat(1,T,V,1)#后三个相机参数，扩展

    z_global = dec_out_all[:, :, :, 2]#取uvd的x,y
    z_global[:, :, 0] = root[:, :, 0, 2]
